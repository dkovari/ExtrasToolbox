...
namespace extras {namespace ParticleTracking {
	...
	void radialavg(const M* img, size_t nRows, size_t nCols, //input image and size
		...
		size_t * Counts = nullptr //optional output array with counts in each bin. Must be same size as imavg
		)
	{
		bool free_counts = (Counts == nullptr);
		if (free_counts) {
			Counts = (size_t*)std::malloc(nAvg * sizeof(size_t));
		}
		...
		try {
			...
			for (size_t xi = max(int(0), int(floor(x0 - Rlim))); xi <= min(int(nCols - 1), int(ceil(x0 + Rlim))); ++xi) {
