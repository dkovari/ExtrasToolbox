function [ph_simple,ph_regularized] = smoothpchip(x,Y,varargin)
...
%
% Outputs:
%   ph_smooth: the smoothed and simplified n-dimensional spline
%   ph_regularized: the smoothed and regularized spline (before simplification)
%
% Options
...
%           Tolerance is compared to the maximum of Err for all points.
%
%   'SmoothingParameter' (default = 1/(1+2*mean(diff(unique(x)))^3) )
...
if isnan(smoothp)
    smoothp = 1/(1+2*mean(diff(unique(x,'sorted')))^3);
end
...
%% Regularize splines
ph_regularized = regularize_pchip_set(pp,p.Results.RegularizationTolerance);

%% Merge into a single spline
ph_regularized = pp2ndim(ph_regularized);

%% Simplify spline to have a minimual number of breaks and still preserve the same shape
ph_simple = simplify_pchip2(ph_regularized,p.Results.SimplifyTolerance);
...
if Err_upper>=TOL
    error('Could not regularize breaks and maintain tolerance.');
end
