classdef ImageLevelsUI < extras.GraphicsChild & extras.uixDerivative
% GUI for interactively setting image color scale limist using a histogram 
% of pixel values
%   Create:
%    obj = ImageLevelsUI(ImageHandle,Parent)
%           ImageHandle: handle to image or imagesc object containing
%           image
...
%           will be closed when obj is deleted.
    ...
    %% Create
    methods
        function this = ImageLevelsUI(ImageHandle,varargin)
            % Create ImageLevels UI for selecting image color scale
            % hUI = extras.ImageLevelsUI(ImageHandle)
            % hUI = extras.ImageLevelsUI(ImageHandle,ParentHandle)
            % hUI = extras.ImageLevelsUI(ImageHandle,'Parent',ParentHandle)
            
            %initiate graphics parent related variables
            this@extras.GraphicsChild(@() figure('Name','Image Levels',...
                    'NumberTitle','off',...
                    'MenuBar','none',...
                    'ToolBar','none'));
    ...
    methods(Hidden=true)
        ...
        function CLimChanged(this)
            
            if ~this.CLimInternalSet
