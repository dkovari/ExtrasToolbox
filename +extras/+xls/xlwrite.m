...
%       If the library is not on the javapath it will be automatically
%       added, provided the jar files are in the folder: './poi_library/'
%
...
%   REVISIONS
% ========================================================================
%   30181115 - Automatic add java libraries (D. Kovari, Emory Univ.)
...
%check for java libraries and load if needed.

% Define the jar file paths
JarPath = fullfile(fileparts(mfilename('fullpath')), 'poi_library');

JarFiles = {'poi-3.8-20120326.jar',...
...
LoadedJars = ismember_partial(JarFiles,JavaInMem);

for n=1:numel(JarFiles)
    if ~LoadedJars(n)
        fprintf('Loading Java Customization: %s\n',JarFiles{n});
        javaaddpath(fullfile(JarPath,JarFiles{n}));
    end
end

end

function TF = ismember_partial(A,B)
assert(ischar(A)||iscellstr(A),'A must be char array or cellstr')
assert(ischar(B)||iscellstr(B),'B must be char array or cellstr')
...
for n=1:numel(A)        
    TF(n) = ~all(cellfun(@isempty,strfind(B,A{n})));
end
end