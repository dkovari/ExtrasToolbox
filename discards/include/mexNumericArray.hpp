...
namespace mex {
	...
	public:
		...
		template<typename M> NumericArray(const NativeArray<M>& src) {
			copyFrom(src);
		}
		template<typename M> NumericArray& operator=(const NativeArray<M>& src) {
			copyFrom(src);
			return *this;
		}
		template<typename M> NumericArray& operator=(NativeArray<M>&& src){
			copyFrom(src);
			return *this;
		}
